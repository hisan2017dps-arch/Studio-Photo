...
import { 
  ...
  Target, Info, Users, Fingerprint
} from 'lucide-react';
import { TRANSLATIONS, MENU_ICONS } from './constants';
import { MenuId, AppLanguage, ChatMessage, RecipeResult } from './types';
import { gemini } from './geminiService';

// Moved outside to prevent re-creation on every render and ensure stable object references
const CAMERA_ANGLES = [
  { 
    id: 'eye', 
    label: 'Eye Level', 
...
const App: React.FC = () => {
  ...
  const STYLE_CATEGORIES = [
    { id: 'family', label: txt.family, icon: <Users className="w-4 h-4"/>, items: [
      ...
      { id: 'fam_batik', label: lang === 'id' ? 'Seragam Batik' : 'Batik Matching', prompt: 'Foto keluarga formal mengenakan seragam batik Indonesia yang serasi, latar belakang pelaminan atau studio mewah, aura profesional dan budaya.' },
    ]},
    ...
    { id: 'business', label: txt.business, icon: <Briefcase className="w-4 h-4"/>, items: [
      ...
      { id: 'biz_pres', label: lang === 'id' ? 'Presentation' : 'Presentation', prompt: 'Suasana bisnis sedang memberikan presentasi, gaya kepemimpinan yang percaya diri, latar belakang ruang rapat atau konferensi.' },
  ...
  const handleRefinePrompt = async () => {
    ...
    try {
      ...
      // Cast the image argument to string | null to resolve potential type inference issues
      const refined = await gemini.refinePrompt(prompt, (refImg || sourceImage2 || null) as string | null, menuTitle, lang);
      setPrompt(refined);
    } catch (err: any) {
  ...
  const generateAction = async () => {
    ...
    try {
      ...
      if (isPortraitMenu && currentSources.length > 0) {
        // ENHANCED ANATOMICAL IDENTITY LOCK PROMPT FOR SINGLE PORTRAIT
        finalPrompt = `[ANATOMICAL IDENTITY CLONE - ACCURACY 100%]
        PROSES UTAMA: HASILKAN GAMBAR DENGAN IDENTITAS WAJAH YANG 100% SAMA DENGAN FOTO REFERENSI.
        DETAIL WAJIB DISALIN PERSIS (IDENTICAL):
        - Garis Wajah: Bentuk rahang, tulang pipi, dan dagu harus identik.
        - Mata: Bentuk mata, lipatan kelopak, warna iris, dan detail bulu mata.
        - Hidung: Batang hidung, lekukan cuping hidung, dan ujung hidung.
        - Alis: Garis pertumbuhan alis, ketebalan, dan bentuknya.
        - Telinga: Bentuk anatomi telinga yang sama persis.
        - Rambut: Tekstur, warna, dan garis rambut (hairline) yang sama.
        - Pipi & Kulit: Tekstur kulit asli, tahi lalat, atau tanda unik lainnya.
        
        KARAKTER DALAM GAMBAR HASIL HARUS ORANG YANG SAMA DENGAN FOTO REFERENSI TANPA MODIFIKASI FITUR WAJAH.
        
        Deskripsi Tambahan: ${prompt}. 
        Angle Kamera: ${anglePrompt}.
        Kualitas: Photorealistic, cinematic lighting, 8k resolution, extreme details.`;
      } else if (sourceTab === 'multi' && multiImages.length > 1) {
        // FIXED MULTI-CHARACTER PROMPT
        finalPrompt = `[MULTI-CHARACTER SCENE GENERATION - DO NOT MERGE FACES]
        DETECTED INPUT: ${multiImages.length} different reference images.
        COMMAND: Create a realistic scene containing exactly ${multiImages.length} distinct people corresponding to the uploaded files.
        
        CRITICAL RULES:
        1. DO NOT blend the faces into one person.
        2. Assign one reference image to one separate character in the scene.
        3. Character 1 must look exactly like the person in Reference Image 1 (Identity Lock).
        4. Character 2 must look exactly like the person in Reference Image 2 (Identity Lock).
        (and so on for all images).
        
        For Wedding/Couple photos: Generate a couple (two distinct people) interacting naturally. Do not merge them into a single hybrid.
        
        Description: ${prompt}.
        Style: Photorealistic, 8k, Ultra-detailed, 100% Facial Identity Match for each person.`;
      }
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await gemini.chat(history as any);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto">
      <div className="lg:col-span-5 space-y-6">
        ...
        <div className={`p-6 rounded-[2.5rem] border ${theme.card} space-y-6 shadow-2xl bg-slate-900/60`}>
          ...
          {(isPortraitMenu || activeMenu === 'character-lab') && source1 && (
            <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-2xl flex items-center gap-4 animate-in zoom-in-95 duration-500">
              <div className="p-3 bg-blue-600 rounded-xl shadow-lg shadow-blue-500/20">
                <Fingerprint className="w-6 h-6 text-white"/>
              </div>
              <div>
                <h4 className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{txt.faceMatch}</h4>
                <p className="text-[9px] text-gray-400 leading-tight mt-0.5">{txt.faceMatchDesc}</p>
              </div>
            </div>
          )}
